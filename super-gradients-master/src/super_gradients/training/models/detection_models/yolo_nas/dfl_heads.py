...
class YoloNASDFLHead(BaseDetectionModule, SupportsReplaceNumClasses):
    def __init__(
        ...
        self.atte = GAM_Attention(inter_channels, inter_channels)
    ...
    def forward(self, x):
        ...
        cls_output = self.atte(cls_output)
        print("cls_output size 2", cls_output.size())

        reg_feat = self.reg_convs(x)
        reg_feat = self.reg_dropout_rate(reg_feat)
        reg_output = self.reg_pred(reg_feat)
        print("reg_output size", reg_output.size())
        #Global Attention Mechanism
        reg_output = self.atte(reg_output)
        print("reg_output size 2", reg_output.size())
...
class h_leakyrelu(nn.Module):
    def __init__(self, inplace=True):
        super(h_leakyrelu, self).__init__()
        self.leakyrelu = nn.LeakyReLU(0.1)

    def forward(self, x):
        return (torch.rand(x.size()).cuda())*self.leakyrelu(x.cuda())

class LearnableLeakyRelu(nn.Module):
    def __init__(self, inplace=True):
        super(LearnableLeakyRelu, self).__init__()
        self.LearnableLeakyRelu = h_leakyrelu()

    def forward(self, x):
        return self.LearnableLeakyRelu(x)
